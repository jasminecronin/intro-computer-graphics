...
void RenderingEngine::RenderScene(const std::vector<Geometry>& objects) {
	...
	//Binds the textures for alpha compositing so they show up correctly on draw
	glActiveTexture(GL_TEXTURE2);
	glBindTexture(GL_TEXTURE_RECTANGLE, scene->background.textureID);
	glActiveTexture(GL_TEXTURE1);
	glBindTexture(GL_TEXTURE_RECTANGLE, scene->foreground.textureID);
