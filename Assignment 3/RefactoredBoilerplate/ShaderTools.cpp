...
GLuint ShaderTools::LinkProgramPoints(GLuint vertexShader, GLuint fragmentShader) {
	// allocate program object name
	GLuint programObject = glCreateProgram();
...
GLuint ShaderTools::InitializePointsShader() {
	// load shader source from files
	std::string vertexSource = LoadSource("../shaders/vertex.glsl");
	std::string fragmentSource = LoadSource("../shaders/fragment.glsl");
	...
	// link shader program
	GLuint program = LinkProgramPoints(vertex, fragment);
